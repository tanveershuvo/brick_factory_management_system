<?php
	...
	$cusid= $_SESSION['customer_id'];
	$sql="SELECT * FROM order_details as o , customer_details as c 
	WHERE o.cus_id=c.cus_id AND o.cus_id = '$cusid' ORDER BY (total_price-paid) DESC";
	$result = $conn->query($sql);
	$array;
	while($row=mysqli_fetch_array($result)){
		$array[] = [
	...
	$sorid= $_SESSION['sordar_id'];
	$sID=$_SESSION['sea_id'];
	$sql="SELECT *  FROM sordar_weekly_bill WHERE sor_id='$sorid' AND sea_id='$sID'";
	$result = $conn->query($sql);
	$array;
	while($row=mysqli_fetch_array($result)){
		$array[] = [
